...
def create_batches(name):
    purchase_master = frappe.get_doc("Quick Purchase", name)
    
    if purchase_master.update_stock:
        ...
        for item in purchase_master.items:
            ...
            if item_doc.has_batch_no:
                try: 
                    ...
                    batch.reference_name = purchase_master.name
        ...
        # frappe.msgprint(f"Batches created for {purchase_master.name}")
...
def create_purchase_invoice(purchase_master):
    purchase_master = frappe.get_doc("Quick Purchase", purchase_master)
