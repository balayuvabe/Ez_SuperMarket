...
def get_item_warehouse_batch_map(filters, float_precision):
	...
	# Filter the items that are expiring within 0 days
    if filters.get("expiring_within_0_days"):
        filtered_iwb_map = {}
        for item_code, warehouses in iwb_map.items():
            for warehouse, batches in warehouses.items():
                for batch_no, qty_dict in batches.items():
                    if qty_dict.expiry_status <= 0:
                        filtered_iwb_map.setdefault(item_code, {}).setdefault(warehouse, {})[batch_no] = qty_dict
        return filtered_iwb_map

    # Filter the items that are expiring within 7 days
    if filters.get("expiring_within_7_days"):
        filtered_iwb_map = {}
        for item_code, warehouses in iwb_map.items():
            for warehouse, batches in warehouses.items():
                for batch_no, qty_dict in batches.items():
                    if 0 < qty_dict.expiry_status <= 7:
                        filtered_iwb_map.setdefault(item_code, {}).setdefault(warehouse, {})[batch_no] = qty_dict
        return filtered_iwb_map

    # Filter the items that are expiring within 15 days
    if filters.get("expiring_within_15_days"):
        filtered_iwb_map = {}
        for item_code, warehouses in iwb_map.items():
            for warehouse, batches in warehouses.items():
                for batch_no, qty_dict in batches.items():
                    if 0 < qty_dict.expiry_status <= 15:
                        filtered_iwb_map.setdefault(item_code, {}).setdefault(warehouse, {})[batch_no] = qty_dict
        return filtered_iwb_map
