...
def get_total_stock(filters):
    ...
    query = query.select(
        ...
        Sum(item.safety_stock).as_("safety_stock")
    ).groupby(item.item_code)

    if filters.get("safety_stock_equal_to_qty"):
        query = query.having(Sum(bin.actual_qty) == Sum(item.safety_stock))
    
    if filters.get("qty_less_than_safety_stock"):
        query = query.having(Sum(bin.actual_qty) <= Sum(item.safety_stock))
